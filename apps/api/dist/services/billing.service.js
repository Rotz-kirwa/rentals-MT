...
class BillingService {
    ...
    static async generateMonthlyInvoices(month, year) {
        ...
        const activeLeases = await database_1.prisma.lease.findMany({
            where: { status: 'ACTIVE' },
        ...
        const existingInvoices = await database_1.prisma.invoice.findMany({
            where: {
                leaseId: { in: leaseIds },
        ...
        const allUnbilledReadings = await database_1.prisma.utilityReading.findMany({
            where: { houseId: { in: houseIds }, isBilled: false },
            include: { utility: true },
        });
        // Group unbilled readings by houseId
        const readingsByHouseMap = new Map();
        ...
        for (const lease of activeLeases) {
            ...
            const rentAmt = (0, financial_js_1.roundMoney)(lease.monthlyRent);
            const garbageAmt = (0, financial_js_1.roundMoney)(lease.house.garbageFee);
            ...
            for (const r of houseReadings) {
                const amt = (0, financial_js_1.roundMoney)(r.totalCharge);
                utilitySubtotal += amt;
                lineItems.push({
            ...
            const grandTotal = (0, financial_js_1.roundMoney)(rentAmt + garbageAmt + utilitySubtotal);
